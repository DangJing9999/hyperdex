...
const ROUTER = require("./HyperdexRouter.json");
const ERC20 = require("./BEP20.json");
const FACTORY = require("./HyperdexFactory.json");
const PAIR = require("./IPancakePair.json");
...
export async function getBalanceAndSymbol(
  ...
  try {
    if (address === tokens.wbnb.address) {
...
export async function swapTokens(
  ...
  console.log(address1,
    ...
    signer)
  */
  const tokensRoute = [address1, address2];
  ...
  const allowance1 = await token1.allowance(accountAddress, routerContract.address)
  if (allowance1 < amountIn) {
    await token1.approve(routerContract.address, amountIn);
  }

  if (address1 === tokens.wbnb.address) {
    // Eth -> Token
  ...
  } else if (address2 === tokens.wbnb.address) {
    // Token -> Eth
